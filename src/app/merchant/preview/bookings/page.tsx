  ...
  return (
    <MerchantPreviewShell merchantName={access.membership.merchantName} merchantSlug={access.membership.merchantSlug} userName={access.user.fullName} userEmail={access.user.email} roleLabel={formatMerchantRole(access.membership.role)} permissions={access.permissions} sites={access.sites} selectedSiteId={selectedSiteId} activeHref="/merchant/bookings" eyebrow="Merchant dashboard" title="Bookings" description="Search, filter, and manage reservations across your assigned sites." actions={<><a href={exportHref} className="rounded-full border border-[var(--line)] bg-white px-4 py-2.5 text-xs font-black">Export CSV ↓</a><Link href="/merchant/bookings/new" className="rounded-full bg-[var(--forest)] px-5 py-2.5 text-xs font-black text-white">+ Create booking</Link></>}>
      ...
      <section className="mt-5 overflow-hidden rounded-2xl border border-[var(--line)] bg-white">
        ...
        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[70rem] text-left text-sm"><thead className="border-b border-[var(--line)] bg-[var(--cream)] text-[0.65rem] uppercase tracking-wider text-[var(--text-muted)]"><tr>{["Booking ID", "Customer", "Site / Court", "Schedule", "Payment", "Status", "Amount", "Actions"].map((label) => <th key={label} className="px-4 py-3 font-black">{label}</th>)}</tr></thead><tbody className="divide-y divide-[var(--line)]">{visibleRows.map((booking) => <tr key={booking.id}><td className="px-4 py-4 font-mono text-xs font-black text-[var(--forest)]">{booking.reference}</td><td className="px-4 py-4"><strong className="block">{booking.customerName ?? "Guest"}</strong><small className="text-[var(--text-muted)]">{booking.customerEmail ?? booking.customerMobileNumber}</small></td><td className="px-4 py-4"><strong className="block">{booking.siteName}</strong><small>{booking.courtName} · {booking.indoor ? "Indoor" : "Outdoor"}</small></td><td className="px-4 py-4 text-xs">{new Intl.DateTimeFormat("en-PH", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Manila" }).format(booking.startsAt)}</td><td className="px-4 py-4"><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-black capitalize">{booking.paymentStatus.replaceAll("_", " ")}</span></td><td className="px-4 py-4"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-black capitalize">{booking.status.replaceAll("_", " ")}</span></td><td className="px-4 py-4 font-black">{formatPeso(booking.totalCents)}</td><td className="px-4 py-4"><Link href={`/merchant/bookings/${booking.id}`} className="rounded-full border border-[var(--line)] px-3 py-2 text-xs font-black">View</Link></td></tr>)}</tbody></table></div>
        <div className="divide-y divide-[var(--line)] md:hidden">{visibleRows.map((booking) => <article key={booking.id} className="p-5"><div className="flex justify-between gap-3"><span className="font-mono text-xs font-black text-[var(--forest)]">{booking.reference}</span><strong>{formatPeso(booking.totalCents)}</strong></div><h2 className="mt-3 font-black">{booking.customerName ?? "Guest"}</h2><p className="mt-1 text-xs text-[var(--text-muted)]">{booking.siteName} · {booking.courtName}</p><p className="mt-3 text-sm">{new Intl.DateTimeFormat("en-PH", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Manila" }).format(booking.startsAt)}</p><div className="mt-4 flex items-center justify-between"><span className="text-xs font-bold capitalize">{booking.paymentStatus.replaceAll("_", " ")} · {booking.status.replaceAll("_", " ")}</span><Link href={`/merchant/bookings/${booking.id}`} className="rounded-full bg-[var(--forest)] px-4 py-2 text-xs font-black text-white">View</Link></div></article>)}</div>
