...
export async function GET(request: Request) {
  ...
  const header = ["Booking ID", "Customer", "Email", "Phone", "Site", "Court", "Start", "End", "Timezone", "Payment Status", "Booking Status", "Amount", "Currency"];
  const body = rows.map((row) => [row.reference, row.customerName, row.customerEmail, row.customerMobileNumber, row.siteName, row.courtName, row.startsAt.toISOString(), row.endsAt.toISOString(), "Asia/Manila", row.paymentStatus, row.status, (row.totalCents / 100).toFixed(2), row.currency]);
