...
export async function POST(request: Request) {
  ...
  if (merchantId !== access.membership.merchantId || !UUID.test(mediaId) || !new Set(["logo", "cover"]).has(kind) || !pathname.startsWith(expectedPrefix)) {
    return NextResponse.json({ error: "Invalid image details." }, { status: 400 });
  }
  const blob = await head(pathname).catch(() => null);
  if (!blob || blob.size > 8 * 1024 * 1024 || !TYPES.has(blob.contentType)) return NextResponse.json({ error: "The uploaded file is not a supported image." }, { status: 400 });
  ...
  try {
    ...
    await del(pathname).catch((cleanupError) => console.error("Merchant media cleanup failed", cleanupError));
  ...
  if (merchant.oldPathname && merchant.oldPathname !== pathname) await del(merchant.oldPathname).catch((error) => console.error("Old merchant media cleanup failed", error));
