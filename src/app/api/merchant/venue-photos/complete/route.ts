...
export async function POST(request: Request) {
  ...
  if (!pathname.includes(expected)) return NextResponse.json({ error: "Invalid photo path." }, { status: 400 });
  ...
  try {
    ...
    await del(pathname).catch((cleanupError) => console.error("Venue photo cleanup failed", cleanupError));
