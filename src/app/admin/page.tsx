...
export default function AdminPage() {
  return (
    <DashboardShell
      ...
      description="The platform control plane for merchants, subscriptions, transaction fees, support, and reconciliation."
      metrics={[
    ...
    >
      <section className="mt-6 overflow-hidden rounded-2xl border border-[var(--line)] bg-white">
        <div className="border-b border-[var(--line)] px-5 py-4">
          ...
          <p className="mt-1 text-xs text-[var(--muted)]">Subscription and platform-fee snapshot</p>
        </div>
        <div className="divide-y divide-[var(--line)]">
          {merchants.map(([merchant, footprint, state, billing]) => (
            <div key={merchant} className="grid gap-2 px-5 py-4 text-sm sm:grid-cols-[1.2fr_1fr_0.7fr_0.7fr] sm:items-center">
              ...
              <span className="text-[var(--muted)]">{footprint}</span>
